...
def read_sequences_from_a_file(input_seqs_dir, filename_with_ext, outputs_dir):
    sequences = []
    flank = "".join(["GC"]*13) # 26 GCs on both sides
    ...
    with open(filepath, "r") as f:
        for i, line in enumerate(f.readlines()):
            ...
            seq = flank+seq+flank
            sequences.append((seq_output_dir, i+1, seq)) # 1-based increamental id for sequences in a file
    return sequences


def read_all_sequences(input_seqs_dir, outputs_dir):
    all_seqs = []
    for i, filename_with_ext in enumerate(os.listdir(input_seqs_dir)):
        seqs = read_sequences_from_a_file(input_seqs_dir, filename_with_ext, outputs_dir)
        all_seqs += seqs
...
def read_input_data(configuration_filepath):
    with open(configuration_filepath, "r") as file:
        ...
        sequences = read_all_sequences(input_seqs_dir, outputs_dir)
    ...
    input_configs = InputConfigs(temperature, sequences, outputs_dir, n_iterations, n_preheating_steps, n_steps_after_preheating, n_nodes)
    print(input_configs)
    return input_configs

# read_input_data("selex/configs/hopper.txt")